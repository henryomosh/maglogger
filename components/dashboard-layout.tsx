...
import { RadioIcon, Users, Calendar, Music, DollarSign, BarChart3, Menu, LogOut, Settings } from "lucide-react"
...
export function DashboardLayout({ children, activeSection = "dashboard", onSectionChange }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-sidebar border-r border-sidebar-border transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-200 ease-in-out lg:translate-x-0`}
      >
                ...
                <DropdownMenuContent className="w-56" align="end">
                  <div className="flex items-center justify-start gap-2 p-2">
                    <div className="flex flex-col space-y-1 leading-none">
                      ...
                      <p className="text-xs text-muted-foreground font-serif capitalize">{user?.role}</p>
                    </div>
                  </div>
                  <DropdownMenuItem>
